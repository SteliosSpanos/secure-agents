...
### 3. Rotating the CloudFront origin secret

The `x-origin-verify` header value (`random_password.origin_secret`) is the token that proves a request reached the origin via CloudFront rather than bypassing it directly. Rotate it if it's ever suspected to have leaked:

```bash
terraform apply -replace=random_password.origin_secret
```

This regenerates the secret, updates the CloudFront distribution's custom origin header, and updates the Lambda authorizer's `ORIGIN_SECRET` env var in one apply. Expect a brief window (typically 1-3 minutes) of `403`s while the CloudFront distribution config propagates to all edge locations.

**Confirm recovery:** tail the authorizer's logs and confirm the `SECURITY ALERT: Request bypassed CloudFront/WAF. Missing or invalid X-Origin-Verify header.` line stops appearing:
